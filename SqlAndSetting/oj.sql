...
CREATE TABLE `discussion` (
  ...
  CONSTRAINT `discussion_ibfk_3` FOREIGN KEY (`avatar`) REFERENCES `user_info` (`avatar`) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE `judge` (
  ...
  `eid` bigint(20) unsigned NOT NULL DEFAULT '0' COMMENT '考试id，非考试题目默认为0',
  `epid` bigint(20) unsigned DEFAULT '0' COMMENT '考试中题目排序id，非考试题目默认为0',
...
CREATE TABLE `exam_record` (
  ...
  `submit_time` datetime NOT NULL COMMENT '具体提交时间',
  `time` bigint(20) unsigned DEFAULT NULL COMMENT '提交时间，为提交时间减去比赛时间',
  `status` int(11) DEFAULT NULL COMMENT '结果',
