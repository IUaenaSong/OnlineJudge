  ...
  return new Promise((resolve, reject) => {
    Vue.prototype.$axios.get(url, {responseType: 'blob'}).then(resp => {
      ...
      if (headers['content-type'].indexOf('json') !== -1) {
        ...
        if (resp.data.error) {
          Vue.prototype.$msg.error(resp.data.error)
        }
        fr.onload = (event) => {
          let data = JSON.parse(event.target.result)
          if (data.msg) {
            Vue.prototype.$msg.info(data.msg)
          } else {
            Vue.prototype.$msg.error('Invalid file format')
          }
        }
      ...
      link.click()
      link.remove()
      Vue.prototype.$msg.success("Downloading...")
      resolve()
    }).catch((error) => {
      reject(error)
    })
...
function downloadFileByText (fileName,fileContent) {
  return new Promise((resolve, reject) => {
      ...
      link.click()
      link.remove()
      Vue.prototype.$msg.success("Download Successfully!")
      resolve()
  })
}
